...
| Tier 4 竞争对齐与创作 | **2 剩余**（原 9：存档槽 ✅、分支树 ✅、聊天内快照 ✅、聊天标签 ✅、内联素材 ✅、Do/Say ✅ 均 2026-09-14 交付；**场景模板填空 ✅ 2026-09-14（`138056b` + `8dd4863`）**；**剩余**＝组合式角色创建（待核）/ 用户脚本（暂缓）；**尾项**＝导出按发言者 ✅ 2026-09-14 已补（`chatTranscript` 现按 `StoredMessage.speakerId` 归属，群场景不再把客人台词记到主角头上））—— 逐项细节见 `TODO.md` 的内联素材进度块 | L344 / L349 / L352 / L361 / L370（另 1 项"组合式角色创建"行号待核） | RisuAI 式内联素材 `{{image::name}}`（零命中）、Do/Say/Narrate 输入模式（零命中）、场景模板填空（零命中）、聊天文件夹/标签（MVP=**标签** ✅ 2026-09-14 **完成且真机验收通过**：`Chat.tags` + `lib/chat/tags.ts` + 6 用例 + `ChatTagBar` + ChatsPanel 过滤条，**服务端零改动**；文件夹按 MVP 决定未做 → 见 **`PLAN-chat-tags.md`**）、用户脚本（Output hook 零命中，**需先定协议边界**） |
