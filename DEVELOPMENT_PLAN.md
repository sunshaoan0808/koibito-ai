...
| ✅ 落地 | 旧版唯一真实待办 `exportWithGrowth()` 成长回写 → **P1-1 已完成**（`19f1111`）；自动时间流逝 → **P1-2 已完成**（`b607209`）；Chaos 事件池扩容 → **P1-3 已完成**（`38bb4c6`）；P2 批次（P2-1~P2-4、P2-6~P2-8）→ **已完成**（`fe21877`），**P2-5 挂起**（本机无生图上游；**代码侧已交付 2026-09-13**，含 OpenAI 形状后端与 `/image` 接线）；**P2-3 有声书半边已交付**（P1-4 通道打通后接上，单文件 MP3） |
...
| 分组 | 未勾数 | 行号 | 处理建议 |
|---|---|---|---|
| §12 平台级（**1/6 交付，5 项待立项**） | 6 | L3086–L3112 | **✅ 分支树视图已交付 2026-09-13**：`src/lib/chat/branchTree.ts` 纯函数（`parentChatId` → 树；**父链指向已删聊天 → 提升为根并单列 `orphaned` 让断链可见**；**父链成环 → 走查遇已访问即停，不可达者提升为根，既不挂死也不丢节点**；`descendants` 计数；按最近活动排序 + id 兜底；`isDescendantOf` 自反短路）+ `BranchTreeView`（只读、按代缩进、点即跳转、当前聊天标记、断链/空态提示）+ `Sidebar` 新导航项（**Ctrl/Cmd-K 自动获得入口**，因其视图清单派生自 `NAV`）。**⚠️ 余 5 项仍需先设计**：存档槽（快照含哪些状态？恢复=覆盖还是 fork？）、NPC 间后台模拟、城镇信息流、知识迷雾、插件/扩展 API —— **已按指示回原项目 `linux4life1/front-porch-AI` 逐项核对，对照结论见 `docs/ABSORPTION-PLAN.md` 第五节「§12 平台级 6 项 × FP 对照」**（要点：存档槽语义照抄 FP 的 per-session 状态 + **fork 复制而非移动**；NPC 互动 FP 的正解是**导演模式**且本仓已有，先盘 `autoAdvance` 勿重造；**城镇信息流／知识迷雾／插件扩展 API 三项 FP 无先例，属真正自研**） |
...
| §15 AI Dungeon 想法 | 6 | L3666–L3767 | ⚠️ 其中 4 项与 P3 Tier 4 重复（用户脚本、Do/Say/Narrate、场景模板、聊天内快照）→ **以 P3 为唯一执行口径**。**P4 独立项实为 8 项，其中 3 项已闭**（§12 分支树已交付；§13 实测代码已存在；§14 实测无剩余差距），**余 §12 的 5 项待立项** |
