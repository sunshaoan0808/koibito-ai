...
| [`plugin-api.md`](./plugin-api.md) | 不改主仓即可扩提示词 section / `/命令` / 视图 | **一期已落地**：`src/lib/plugins/registry.ts`（7 用例，纯核注册表）；二/三期未做 |
...
- **plugin-api（独立，可并行）**：一期（纯核注册表：`types.ts` + `registry.ts`）**已落地**。接入点仍只有一个（`buildPrompt`），风险集中在信任模型，故一二期明确**不做动态加载、不做沙箱**——"进程内＝全权限"必须如实写在界面文案与插件面板里。二期接 `buildPrompt` ＋ `/命令`。
